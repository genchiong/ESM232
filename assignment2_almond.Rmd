...
yield_plot <- ggplot(yield_anomalies, aes(x = year, y = yield)) +
  geom_line() +
  labs(title = "Annual Almond Yield Anomalies", y = "Yield (tons/acre)", x = "Year") +
  theme_minimal() +
...
precip_plot <- ggplot(sum_precip, aes(x = year, y = sum_precip)) +
  geom_line() +
  labs(title = "January Precipitation", y = "Precipitation (mm)", x = "Year") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))

temp_plot <- ggplot(mean_min_temp, aes(x = year, y = mean_min_temp)) +
  geom_line() +
  labs(title = "February Mean Minimum Temperature", y = "Mean Minimum \n February Temperature (ºC)", x = "Year") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
```
